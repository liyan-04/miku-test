#!/bin/bash
#
# pull-hs1.vzan.com 拉流测试脚本
#
# 用法:
#   ./vzan_pull_hs1.sh --stream <path/stream> [--tag <tag>] [--duration <秒>] [--ip <ip>]
#   ./vzan_pull_hs1.sh --stream hls2/stream --tag liyan
#   ./vzan_pull_hs1.sh --stream hls2/stream --ip 10.210.32.28
#
...
必填参数:
  --stream  流路径，如 hls2/stream

可选参数:
  --tag      查询参数标签（默认 liyan）
  --ip       直接指定 IP（避免配置 hosts），格式: --ip 10.210.32.28
  --duration 拉流探测时长（默认 5 秒）

示例:
  ./vzan_pull_hs1.sh --stream hls2/stream --tag liyan
  ./vzan_pull_hs1.sh --stream hls2/stream --ip 10.210.32.28 --duration 10
EOF
}

# ========== 参数默认值 ==========
STREAM=""
TAG="jftest$(date +%Y%m%d%H%M%S)"
DURATION=5
DOMAIN="pull-hs1.vzan.com"
CUSTOM_IP=""
...
# ========== 构建拉流 URL ==========
URL_HLS="http://${DOMAIN}/${STREAM}.m3u8?testtag=${TAG}"

log_step "测试 pull-hs1.vzan.com HLS 拉流..."
log_info "域名: $DOMAIN"
